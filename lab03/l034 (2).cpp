...
Segment* randomize() {
    ...
    unordered_map<SubSquare, vector<Point>, MyHashFunction> umap;
    ...
    for (int i = 0; i < (int)vector3.size(); i++) {
        ...
        for (int a = squareST.getX() - 2; a <= squareST.getX() + 2; a++) {
            for (int j = squareST.getY() - 2; j <= squareST.getY() + 2; j++) {
                ...
                if (umap.find(tempSq) != umap.end()) {  
                    for(Point p : umap.find(tempSq)->second) {
                        double dist = distance(p.getX(), vector3[i].getX(), p.getY(), vector3[i].getY());
                        if (dist < lDist) {
                            ret->setA(p);
                            ret->setB(vector3[i]);
                            lDist = dist;
        ...
        if (lDist < delta) {
            ...
            umap.clear();
            for (int k = 0; k <= i; k++) {
                SubSquare squareNew = SubSquare(vector3[k].getX() / (delta / 2), vector3[k].getY() / (delta / 2));
                if (umap.find(squareNew) == umap.end()) {
                    umap.insert({squareNew, vector<Point>()});
                }
                umap.find(squareNew)->second.push_back(vector3[k]);
            }
        }
        else {
            if (umap.find(squareST) == umap.end()) {
                umap.insert({squareST, vector<Point>()});
            }
            umap.find(squareST)->second.push_back(vector3[i]);
        }
    }
    return ret;
}
