...
Point findLowestY(vector<Point> pts) {
    Point retP = pts[0];
    for (unsigned int i = 1; i < pts.size(); i++) {
        if (pts[i].getY() < retP.getY()) {
            retP = pts[i];
        }
    }
    return retP;
...
bool sortPolar(Point A, Point B) {
    double checkA = atan2(A.getY(), A.getX());
    double checkB = atan2(B.getY(), B.getX());
...
stack<Point> grahamScan(vector<Point>& pointSet) {
    stack<Point> retStack;
    Point p0 = findLowestY(pointSet);
    swap(pointSet[0], p0);

    sort(pointSet.begin() + 1, pointSet.end(), sortPolar);
    retStack.push(pointSet[0]);
    retStack.push(pointSet[1]);

    for (unsigned int i = 2; i < pointSet.size(); i++) {
        Point BCheck = retStack.top();
        retStack.pop();
        Point ACheck = retStack.top();

        while (retStack.size() > 1 && counterClockWise(ACheck, BCheck, pointSet[i]) <= 0) {
            BCheck = retStack.top();
            retStack.pop();
            ACheck = retStack.top();
        }
        retStack.push(BCheck);
        if(!(BCheck.getX() == pointSet[i].getX()))
            retStack.push(pointSet[i]);
    }

    stack<Point> tem = retStack;
...
void part1() {
    ...
    writeFile("quickHull.ppm");
}

void part2() {
    Point lineSecPop, linePop;
 //  if (points.size() == 0)
    points = part0();
    stack<Point> grahamFinal = grahamScan(points);     
    ...
    unsigned int i = 0;
    while (i < grahamFinal.size())
    {
        linePop = grahamFinal.top();
        grahamFinal.pop();
        lineSecPop = grahamFinal.top();
        line(length * linePop.getX(), length * linePop.getY(), length * lineSecPop.getX(), length * lineSecPop.getY());
        i++;
    }

    writeFile("grahamScan.ppm");
